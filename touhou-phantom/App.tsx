...
const App: React.FC = () => {
  ...
  const generateUpgradeOptions = useCallback((currentWeapons: Weapon[], currentPassives: string[], currentHp: number, maxHp: number, currentStats: any): UpgradeOption[] => {
    const options: UpgradeOption[] = [];
    const maxOptions = 3;

    // Check slot limits
    const weaponSlotsAvailable = (currentStats.weaponSlots || 6) - currentWeapons.length;
    const passiveSlotsAvailable = (currentStats.passiveSlots || 6) - currentPassives.length;

    // Pools
    const existingWeaponPool = currentWeapons.filter(w => w.level < w.maxLevel);

    // Filter New Weapons: Must NOT be exclusive to another character AND have available weapon slots
    const newWeaponPool = weaponSlotsAvailable > 0 ? Object.values(WEAPON_DEFS).filter(def => {
        const hasIt = currentWeapons.some(cw => cw.id === def.id);
        if (hasIt) return false;
        if (def.exclusiveTo && def.exclusiveTo !== selectedCharacter.id) return false;
        // Exclude evolved weapons from direct pickup
        if (def.id.includes('_evolved')) return false;
        return true;
    }) : [];

    // Filter Passives: Must have available passive slots
    const passivePool = passiveSlotsAvailable > 0 ? Object.values(PASSIVE_DEFS).filter(p => {
        if (currentPassives.includes(p.id)) return false;
        if (p.exclusiveTo && p.exclusiveTo !== selectedCharacter.id) return false;
        return true;
    }) : [];
    ...
    // Fantasy Gift probability: 1% + (luck - 1.0) * 50%
    const fantasyGiftChance = 0.01 + Math.max(0, (currentStats.luck - 1.0) * 0.5);
    ...
    while(options.length < maxOptions && safetyCounter < 50) {
        ...
        // 1. Fantasy Gift (luck-scaled probability)
        if (rand < fantasyGiftChance) {
             candidate = {
                id: 'fantasy_gift', type: 'heal', name: '幻想乡的馈赠', description: '全额回血 + 生命上限提升 + 幸运+10%',
  ...
  const handleLevelUp = (currentWeapons: Weapon[], currentPassives: string[], hp: number, maxHp: number, time: number, currentStats: any) => {
      gameTimeRef.current = time;
      setGameState(GameState.PAUSED_LEVEL_UP);
      const options = generateUpgradeOptions(currentWeapons, currentPassives, hp, maxHp, currentStats);
